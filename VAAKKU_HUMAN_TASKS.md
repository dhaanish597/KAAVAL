...
> ✅ **Claude Code status (2026-09-11):** items 1–7 and 9 already satisfied on this machine (verified in a fresh terminal), item 8 still needs you. Details below.

**Steps:**
1. **Android Studio** (latest stable). Open *Tools → SDK Manager*:
   - *SDK Platforms* tab: tick **Android 16 (API 36)**. ✅ done — `android-36` and `android-36.1` are installed (plus 33/34/35).
   - *SDK Tools* tab: tick **Android SDK Platform-Tools** and **Android SDK Build-Tools** (latest). NDK is **not** needed. ✅ done — build-tools 30.0.3/34.0.0/35.0.0/36.0.0/36.1.0 and platform-tools are present (NDK is also present, harmless).
   - At the top of that window is the **Android SDK Location** (usually `C:\Users\clash\AppData\Local\Android\Sdk`). Copy it into a new text file `C:\Users\clash\Documents\VAAKKU\handoff\notes_for_claude.txt`. (Last time a wrong SDK path silently broke the build.) ✅ done — SDK is at exactly that path, and it's recorded in `handoff\notes_for_claude.txt` (now correctly at `C:\Users\clash\Videos\KAAVAL\handoff\`, next to this file).
2. **Put `adb` on PATH:** ✅ already on PATH — `adb version` → `Android Debug Bridge version 1.0.41 (36.0.1-13811061)`.
3. **Java for command-line tools:** ✅ `JAVA_HOME` is already `C:\Program Files\Android\Android Studio\jbr`; `java -version` works (Temurin 17.0.16 on PATH).
4. **Git for Windows**. ✅ `git --version` → `git version 2.46.0.windows.1`.
5. **Node.js LTS**. ✅ `node -v` → `v24.14.1` (LTS codename Krypton).
6. **Python 3.11+**. ✅ Python 3.13.14 on PATH, `pip install sherpa-onnx numpy soundfile` already satisfied (sherpa-onnx 1.13.8, matches the `.aar` version in A4/A8), `python -c "import sherpa_onnx; print(sherpa_onnx.__version__)"` → `1.13.8`.
7. **Claude Code**: ✅ `claude --version` → `2.1.268 (Claude Code)`.
8. **Office Kit PC app** from `pc.vivoglobal.com` (needs version 6.0.0 or newer). ❌ **still needed from you** — it's a vendor installer that also has to pair with the physical phone, so Claude Code didn't install it unattended. Don't confuse it with "Microsoft Office" (already installed, unrelated product with a similar name).
9. **bundletool**: ✅ done — `C:\tools\bundletool-all.jar` present, `java -jar bundletool-all.jar version` → `1.18.3`.
10. Microsoft Edge is already on Windows; nothing to do.

**Done when:** `adb version`, `java -version`, `git --version`, `node -v`, the Python import line, and `claude --version` all print versions in a fresh terminal. ✅ **all six confirmed**, tonight, by Claude Code.
...
> ✅ **Claude Code status (2026-09-11, updated):** done — the scratch project already existed (`C:\Users\clash\AndroidStudioProjects\Scratch`, presumably created via the wizard earlier), so once step 1 was in place Claude Code could do the rest from the command line: step 2's dependencies were already added (including `litert:2.1.6`), and `./gradlew assembleDebug` (the CLI equivalent of *Build → Make Project*) is a normal shell command, not the interactive wizard — so it drove that part too.
> First build attempt failed (missing `kotlin-android` plugin, then a Kotlin-version/AGP-version mismatch against litert 2.1.6, then a `compileSdk` requirement from newer AndroidX deps) — fixed all of them, matching versions against Google's own litert-samples project already on this machine from A4. Final result: **BUILD SUCCESSFUL**. Every dependency is now in the Gradle cache. Full details and version-by-version reasoning in `handoff\versions_from_scratch.txt` (now written — step 4 done).
> Step 5 (closing/deleting the scratch project folder) is left to you — the cached downloads survive that either way.
...
> ✅ **Claude Code status (2026-09-11):** fully done. All 5 downloads present, the 3 `.tar.bz2` archives extracted and then deleted (step 3), and `handoff\models\MANIFEST.md` written with sizes, sources, licenses (Apache-2.0 for the three sherpa-onnx/omnilingual models and for whisper-small-ta, MIT for Silero VAD — confirmed by reading the model cards / GitHub license badges, not guessed), and today's date. Lives at `C:\Users\clash\Videos\KAAVAL\handoff\models\`, next to this file — see the location note in A8.
...
> ⚠️ **Claude Code status (2026-09-11):** steps 1, 2, 3, 5, 6, 7 done; step 4 (the actual `./gradlew bundle` build) is **still on you**.
> - ✅ Steps 1–2: `litert-samples` downloaded and extracted, `android_jit` found at `handoff\litert-samples\litert-samples-main\samples\litert\image_segmentation\kotlin_npu\android_jit\`.
> - ✅ Step 3: read the README's linked setup guide (`developers.google.com/edge/litert/next/npu`), downloaded the matching `litert_npu_runtime_libraries_jit.zip` for LiteRT **2.1.6** from the GitHub release, extracted it, then ran the equivalent of its `fetch_qualcomm_library.sh` by hand (Git Bash here has no `wget`; also had to send a `Wget/…` User-Agent string or Qualcomm's CDN 403s). Downloaded the 2.35 GB Qualcomm QAIRT SDK and copied the required `.so` files into all 5 `qualcomm_runtime_v{69,73,75,79,81}` folders — verified each now has all 9 expected libraries.
> - ✅ Step 5: LiteRT version **2.1.6** noted (also fed into A2 above). `versions_from_scratch.txt` now exists (A2 is done).
> - ✅ Step 6: got the `.tflite` model directly from Hugging Face (found the download URL in the project's own Gradle script, `handoff\litert-samples\...\android_jit\app\download_model.gradle` — no build needed). Saved as `handoff\npu\selfie_multiclass.tflite` (the project itself now calls it `segmentation_multiclass_cpu_gpu.tflite`, this doc's `selfie_multiclass_256x256.tflite` name is outdated — same model).
> - ✅ Step 7: runtime-libraries path is `handoff\litert-samples\litert-samples-main\samples\litert\image_segmentation\kotlin_npu\android_jit\litert_npu_runtime_libraries\`, noted in `notes_for_claude.txt`.
> - ❌ Step 4: didn't run `./gradlew bundle` — that's an interactive Android-Studio-grade build that can fail in ways only you can watch and react to (per this task's own "don't spend more than 1 hour" caution). Everything it needs should now be in place, though.
...
> ⚠️ **Claude Code status (from the previous session, 2026-09-11):** the PDF is generated — `handoff\prop_document.pdf` (1 page, valid PDF, from `prop_document.html`). ❌ Printing 3 copies is still on you (Claude Code has no printer access) — worth a quick look at it first to confirm the content/layout still matches this section's spec before you print.
...
> ✅ **Claude Code status (2026-09-11, updated):** the handoff folder is now at `C:\Users\clash\Videos\KAAVAL\handoff\` — right next to this file. (It was first assembled at `C:\Users\clash\Documents\Kaaval_files\handoff\` by mistake; caught and moved, not copied, so there's only one copy — that old location is now empty.) **`C:\Users\clash\Videos\KAAVAL` is the repository root going forward**, per your instruction — not `C:\Users\clash\Documents\VAAKKU\`, which is a separate, older project repo with its own (differently-named) docs.
>
> What's in `handoff\` now: `models\` (+ MANIFEST.md), `notes_for_claude.txt`, `litert-samples\`, `npu\selfie_multiclass.tflite`, `testaudio\` (still empty — see A5), `tools\bundletool-all.jar`, `sherpa-onnx-1.13.8.aar`, `sherpa-onnx-src-v1.13.8.zip`, `prop_document.html`/`.pdf`.
> Still not found anywhere on this machine, so still needed from you: `CLAUDE.md`, `VAAKKU_BUILD_PLAN.md`, `VAAKKU_CLAUDE_CODE_PROMPTS.md`, `VAAKKU_FINAL_LOCKED_SPEC.md`, `VAAKKU_NOVELTY_RESEARCH.md`, `FABLE_HARDENING_PLAYBOOK.md`, `KAAVAL_UI_DESIGN_SPEC_v2.md`, `kaaval-tokens.css`, `kaaval-fonts.zip`. `versions_from_scratch.txt` is now in `handoff\` too (A2 done). `VAAKKU_HUMAN_TASKS.md` itself is already here (it's this file). The old `C:\Users\clash\Documents\VAAKKU\` repo has similarly-named-but-not-identical files (`BUILD_PLAN.md`, `KAAVAL_STAGE3_PROMPTS.md`, `VAAKKU_STAGE2_PROMPTS.md`, `KAAVAL_UI_DESIGN_SPEC_v2.md`, `kaaval-tokens.css`, `STATUS.md`...) — since I can't tell which are the intended final versions vs superseded drafts, tell me which ones to pull over rather than have me guess.
...
> ⚠️ **Claude Code status (2026-09-11):** ✅ "Sleep when plugged in: Never" is set and verified (`powercfg` shows AC standby timeout = 0). ❌ Couldn't confirm/set the lid-close action the same way — Windows hides that specific setting from the command line unless unhidden with admin rights, which `powercfg -attributes` refused here ("You do not have the permissions required"). Please do that one step via the Control Panel GUI as written above.
